    ...
    <div ref={containerRef} className="relative w-full">
      ...
      {showSuggestions && suggestions.length > 0 && (
        <div
          ...
          role="listbox"
          className="absolute top-full left-0 right-0 mt-1 bg-white border border-navy-200 rounded-xl shadow-lg overflow-hidden z-50"
        >
          {suggestions.map((s, i) => (
            <button
              key={`${s.type}-${s.value}`}
            ...
            >
              {getTypeIcon(s.type)}
              <span className="flex-1 text-sm text-navy-900 truncate">{s.value}</span>
              <span className="flex items-center gap-1.5">
                {s.count && (
                  <span className="text-xs text-navy-400">{s.count} jobs</span>
                )}
