  ...
  useEffect(() => {
        async function fetchSimilar() {
                try {
                          ...
                          // Silently fail - similar jobs is non-critical
                } finally {
                          setLoading(false)
                }
        }
                          ...
                          const salary = formatSalary(job.salary_min, job.salary_max)
                                      return (
                                                    <Link key={job.id} href={`/jobs/${job.id}`}>
                                                                  <div className="group rounded-lg border border-navy-100 bg-white p-4 transition hover:shadow-md hover:border-navy-300 hover:-translate-y-0.5">
                                                                                  <div className="flex items-start gap-3">
                                                                                    {job.company?.logo_url ? (
                                                                          <img
                                                                                                  src={job.company.logo_url}
                                                                                                  alt={job.company.name}
                                                                                                  className="h-8 w-8 rounded-lg object-contain flex-shrink-0 border border-navy-100"
                                                                                                />
                                                                        ) : (
